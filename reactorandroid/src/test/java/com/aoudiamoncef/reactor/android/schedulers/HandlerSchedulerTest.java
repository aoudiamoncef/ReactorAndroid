...
import com.aoudiamoncef.reactor.android.testutil.CountingRunnable;
...
public final class HandlerSchedulerTest {
    ...
    public void directScheduleOnceWithNegativeDelayPostsImmediately() {
        CountingRunnable counter = new CountingRunnable();
        scheduler.schedule(counter, -1, TimeUnit.MINUTES);
    ...
    public void directScheduleOnceUsesHook() {
        ...
        RxJavaPlugins.setScheduleHandler((Function<Runnable, Runnable>) runnable -> {
    ...
    public void directScheduleOnceDisposedDoesNotRun() {
        CountingRunnable counter = new CountingRunnable();
        Disposable disposable = scheduler.schedule(counter);
        disposable.dispose();
    ...
    public void directScheduleOnceWithDelayPostsWithDelay() {
        CountingRunnable counter = new CountingRunnable();
        scheduler.schedule(counter, 1, MINUTES);
    ...
    public void directScheduleOnceWithDelayUsesHook() {
        final CountingRunnable newCounter = new CountingRunnable();
        final AtomicReference<Runnable> runnableRef = new AtomicReference<>();
        ReactorPlugins.setScheduleHandler((Function<Runnable, Runnable>) runnable -> {
            runnableRef.set(runnable);
            return newCounter;
        });
    ...
    public void directScheduleOnceWithDelayDisposedDoesNotRun() {
        CountingRunnable counter = new CountingRunnable();
        Disposable disposable = scheduler.schedule(counter, 1, MINUTES);
    ...
    @Test @Ignore("Implementation delegated to default Reactor implementation")
    public void directSchedulePeriodicallyReschedulesItself() {
        CountingRunnable counter = new CountingRunnable();
        scheduler.schedulePeriodically(counter, 1, 1, MINUTES);
    ...
    public void directSchedulePeriodicallyDisposedDoesNotRun() {
        CountingRunnable counter = new CountingRunnable();
        Disposable disposable = scheduler.schedulePeriodically(counter, 1, 1, MINUTES);
    ...
    @Test
    public void throwingActionRoutedToReactorPlugins() {
        Consumer<? super Throwable> originalErrorHandler = ReactorPlugins.getErrorHandler();

        try {
            final AtomicReference<Throwable> throwableRef = new AtomicReference<>();
            ReactorPlugins.setErrorHandler((Consumer<Throwable>) throwableRef::set);
            ...
            ReactorPlugins.setErrorHandler(originalErrorHandler);
        }
    }
    ...
    public void directScheduleOnceInputValidation() {
        ...
        try {
            scheduler.schedule(null, 1, MINUTES);
            fail();
        } catch (NullPointerException e) {
            assertEquals("run == null", e.getMessage());
        }
        try {
            scheduler.schedule(new CountingRunnable(), 1, null);
            fail();
        } catch (NullPointerException e) {
    ...
    public void directSchedulePeriodicallyInputValidation() {
        try {
            scheduler.schedulePeriodically(null, 1, 1, MINUTES);
            fail();
        } catch (NullPointerException e) {
            assertEquals("run == null", e.getMessage());
        }
        try {
            scheduler.schedulePeriodically(new CountingRunnable(), 1, -1, MINUTES);
            fail();
        } catch (IllegalArgumentException e) {
            assertEquals("period < 0: -1", e.getMessage());
        }
        try {
            scheduler.schedulePeriodically(new CountingRunnable(), 1, 1, null);
            fail();
        } catch (NullPointerException e) {
            assertEquals("unit == null", e.getMessage());
        }
